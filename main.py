...
class ChatBot(object):
    def __init__(self):
        inputs, target_inputs, targets = get_data()
    ...
    def tokenize_decoder(self):
        ...
        print("One Hot Outputs of Decoder Shape : ",self.Ydecoder_pad.shape)
    ...
    def training_pase(self):
        ...
        # make decoder but before that need to build ChatBot so only specify embedding layer
        ...
        dense2 = Dense(d2, name='dense2_attention', activation=ChatBot.softmax_over_time)
        ...
        for t in range(self.max_length_decoder):
            ...
            seq_layer = Lambda(lambda x: x[:, t:t+1]) # how ever we pass the entire target sequence to decoder we do teacher forcing word by word.
    ...
    def train_model(self):
        print("Training ChatBot Network !!!")
        self.training_pase()
    ...
    def save_model(self):
        ...
        with open(chatbot_path, "w") as json_file:
            json_file.write(model_json)
        self.attention_network.save_weights(chatbot_weights)
        self.attention_network.compile(
                                    loss = ChatBot.custom_loss,
                                    optimizer=Adam(0.001),
                                    metrics=[ChatBot.acc] )
    ...
    def load_model(self):
        print("Loading ChatBot Network !!!")
        json_file = open(chatbot_path, 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        loaded_model = model_from_json(loaded_model_json, custom_objects={'softmax_over_time': ChatBot.softmax_over_time})
        loaded_model.load_weights(chatbot_weights)
    ...
    def language_translation(self):
        ...
        while True:
            ...
            print("True Question: ", true_input_sentence)
            print("Answer: ", input_sentence)
            print("Chatbot answer: ", english_sentence)
            print("-------------------------------------------------------------------------------------------")
...
if __name__ == "__main__":
    ...
    get_custom_objects().update({'softmax_over_time': Activation(ChatBot.softmax_over_time)})
    if not os.path.exists(chatbot_path) or not os.path.exists(chatbot_weights):
        model.train_model()
    model.load_model()
